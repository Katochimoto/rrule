...
;(function (root, factory) {
  ...
  dateutil.DateTime = function (...args) {
    if (args.length === 1 && args[0] instanceof dateutil.DateTime) {
      ...
      this._date = new (Function.prototype.bind.apply(dateutil.DateTime.strategy, [null].concat(args)))();
    }
  };

  dateutil.DateTime.strategy = Date;

  dateutil.DateTime.interface = ['getDate', 'getDay', 'getFullYear', 'getHours', 'getMinutes', 'getMonth', 'getSeconds', 'getTime', 'getTimezoneOffset', 'getUTCDate', 'getUTCFullYear', 'getUTCHours', 'getUTCMinutes', 'getUTCMonth', 'getUTCSeconds', 'toString', 'valueOf'].reduce(function (data, name) {
    data[name] = {
      value: function (...args) {
        var _date;

        return (_date = this._date)[name].apply(_date, args);
      }
    };
    return data;
  }, {});
  ...
  dateutil.DateTime.UTC = function (...args) {
    var _dateutil$DateTime$st;

    return (_dateutil$DateTime$st = dateutil.DateTime.strategy).UTC.apply(_dateutil$DateTime$st, args);
  };

  dateutil.DateTime.parse = function (...args) {
    var _dateutil$DateTime$st2;

    return (_dateutil$DateTime$st2 = dateutil.DateTime.strategy).parse.apply(_dateutil$DateTime$st2, args);
  };
  ...
  dateutil = _extends({}, dateutil, {
    ...
    getYearDay: function (date) {
      var dateNoTime = new dateutil.DateTime(date.getFullYear(), date.getMonth(), date.getDate());
      return Math.ceil((dateNoTime - new dateutil.DateTime(date.getFullYear(), 0, 1)) / dateutil.ONE_DAY) + 1;
    },
    ...
    fromOrdinal: function (ordinal) {
      ...
      return new dateutil.DateTime(dateutil.ORDINAL_BASE.getTime() - dateutil.tzOffset(dateutil.ORDINAL_BASE) + millisecsFromBase + dateutil.tzOffset(new dateutil.DateTime(millisecsFromBase)));
    },
    ...
    combine: function (date, time) {
      time = time || date;
      return new dateutil.DateTime(date.getFullYear(), date.getMonth(), date.getDate(), time.getHours(), time.getMinutes(), time.getSeconds(), time.getMilliseconds());
    },
    ...
    untilStringToDate: function (until) {
      ...
      return new dateutil.DateTime(dateutil.DateTime.UTC(bits[1], bits[2] - 1, bits[3], bits[5] || 0, bits[6] || 0, bits[7] || 0));
    }
  ...
  RRule.prototype = {
    ...
    */
    ...
    _cacheAdd: function (what, value, args) {
      ...
      if (value) {
        value = value instanceof dateutil.DateTime ? dateutil.clone(value) : dateutil.cloneDates(value);
      }
  ...
  IterResult.prototype = {
    ...
    init: function (method, args) {
      ...
      if (method === 'between') {
        this.maxDate = args.inc ? args.before : new dateutil.DateTime(args.before.getTime() - 1);
        this.minDate = args.inc ? args.after : new dateutil.DateTime(args.after.getTime() + 1);
  ...
  RRuleStr.prototype._handle_COUNT = RRuleStr.prototype._handle_int;['_handle_BYSETPOS', '_handle_BYMONTH', '_handle_BYMONTHDAY', '_handle_BYYEARDAY', '_handle_BYEASTER', '_handle_BYWEEKNO', '_handle_BYHOUR', '_handle_BYMINUTE', '_handle_BYSECOND'].forEach(function (method) {
