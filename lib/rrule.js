...
function RDateTime () {
  ...
  if (args.length === 1 && args[0] instanceof RDateTime) {
    ...
    this._date = new (Function.prototype.bind.apply( RDateTime.Strategy, [ null ].concat( args) ));
  }
}
...
RDateTime.UTC = function () {
  var args = [], len = arguments.length;
  while ( len-- ) args[ len ] = arguments[ len ];

  return (ref = RDateTime.Strategy).UTC.apply(ref, args)
  var ref; // eslint-disable-line new-cap
};

RDateTime.parse = function () {
  var args = [], len = arguments.length;
  while ( len-- ) args[ len ] = arguments[ len ];

  return (ref = RDateTime.Strategy).parse.apply(ref, args)
  var ref;
};
...
RDateTime.setStrategy = function (Strategy) {
  ...
  for (var name in RDateTime.interface) {
 ...
 */
var ORDINAL_BASE = new RDateTime(1970, 0, 1);
...
var CallbackIterResult = (function (IterResult) {
  ...
  var staticAccessors = { allowedMethods: { configurable: true } };
...
Iterinfo.prototype.rebuild = function (year, month) {
  ...
  if (year !== this.lastyear) {
    ...
    var wday = getWeekday(new RDateTime(year, 0, 1));
    ...
    if (!plb(rr.options.byweekno)) {
      ...
      if (no1wkst) {
        ...
        var lnumweeks;
        if (!contains(rr.options.byweekno, -1)) {
          var lyearweekday = getWeekday(new RDateTime(year - 1, 0, 1));
...
Iterinfo.prototype.wdayset = function (year, month, day) {
  ...
  var i = toOrdinal(new RDateTime(year, month - 1, day)) - this.yearordinal;
  var start = i;
  for (var j = 0; j < 7; j++) {
    set[i] = i;
    ++i;
...
RRuleSet.prototype = {
  ...
  _iter: function (iterResult) {
    ...
    for (var i = 0; i < this._rdate.length; i++) {
      if (!iterResult.accept(new RDateTime(this$1._rdate[i]))) {
        break
      }
    }
  ...
  clone: function () {
    ...
    for (i = 0; i < this._rdate.length; i++) {
      rrs.rdate(new RDateTime(this$1._rdate[i]));
    }
    for (i = 0; i < this._exrule.length; i++) {
      rrs.exrule(this$1._exrule[i].clone());
    }
    for (i = 0; i < this._exdate.length; i++) {
      rrs.exdate(new RDateTime(this$1._exdate[i]));
    }
    return rrs
  }
};
